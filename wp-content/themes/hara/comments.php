...
<div id="comments" class="comments-area">
	...
	<?php if (have_comments()) : ?>
        <h3 class="comments-title"><?php comments_number(esc_html__('0 تعليق', 'hara'), esc_html__('1 تعليق', 'hara'), esc_html__('٪ تعليقات', 'hara')); ?></h3>
    ...
    ?>
		<p class="no-comments"><?php esc_html_e('التعليقات مغلقة.', 'hara'); ?></p>
	<?php endif; ?>
	...
	<?php
        ...
        $comment_args = array(
        'title_reply'=> esc_html__('اترك رد', 'hara'),
        'comment_field' => '<p class="comment-form-comment form-group"><textarea id="comment" placeholder = "'.esc_attr__('تعليقك*','hara').'" class="form-control" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
        'fields' => apply_filters(
            ...
            'label_submit' => esc_html__('أضف تعليقا', 'hara'),
            'comment_notes_before' => '<div class="form-group h-info">'.esc_html__('لن يتم نشر عنوان بريدك الإلكتروني. الحقول المطلوبة هي الماركات.', 'hara').'</div>',
            'comment_notes_after' => '',
        );
    ?>
