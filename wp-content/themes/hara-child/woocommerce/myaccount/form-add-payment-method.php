	...
	<form id="add_payment_method" method="post">
		<div id="payment" class="woocommerce-Payment">
			...
			<div class="form-row">
				...
				<button type="submit" class="woocommerce-Button woocommerce-Button--alt button alt<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" id="place_order" value="<?php esc_attr_e( 'Add payment method', 'woocommerce' ); ?>"><?php esc_html_e( 'إضافة طريقة دفع', 'woocommerce' ); ?></button>
				<input type="hidden" name="woocommerce_add_payment_method" id="woocommerce_add_payment_method" value="1" />
			</div>
		</div>
	</form>
<?php else : ?>
	<?php wc_print_notice( esc_html__( 'لا يمكن إضافة طرق الدفع الجديدة إلا أثناء الخروج. يرجى الاتصال بنا إذا كنت بحاجة إلى المساعدة.', 'woocommerce' ), 'notice' ); ?>
<?php endif; ?>
