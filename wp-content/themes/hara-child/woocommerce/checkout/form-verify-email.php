...
<form name="checkout" method="post" class="woocommerce-form woocommerce-verify-email" action="<?php echo esc_url( $verify_url ); ?>" enctype="multipart/form-data">
	...
	if ( $failed_submission ) {
		wc_print_notice( esc_html__( 'لم نتمكن من التحقق من عنوان البريد الإلكتروني الذي قدمته. حاول مرة اخرى.', 'woocommerce' ), 'error' );
	}
	?>
	<p>
		<?php
		printf(
			...
			esc_html__( 'لعرض هذه الصفحة، يجب عليك إما %1$slogin%2$s أو التحقق من عنوان البريد الإلكتروني المرتبط بالطلب.', 'woocommerce' ),
	...
	<p class="form-row">
		<label for="email"><?php esc_html_e( 'عنوان البريد الإلكتروني', 'woocommerce' ); ?>&nbsp;<span class="required">*</span></label>
	...
	<p class="form-row">
		<button type="submit" class="woocommerce-button button <?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ); ?>" name="verify" value="1">
			<?php esc_html_e( 'يؤكد', 'woocommerce' ); ?>
		</button>
	</p>
</form>
