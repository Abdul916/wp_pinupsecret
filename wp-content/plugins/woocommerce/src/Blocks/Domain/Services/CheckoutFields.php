...
class CheckoutFields {
	...
	public function __construct( AssetDataRegistry $asset_data_registry ) {
		...
		$this->core_fields         = [
			'email'      => [
				'label'          => __( ' عنوان البريد الإلكتروني ', 'woocommerce' ),
				'optionalLabel'  => __(
			...
			'first_name' => [
				'label'          => __( 'الاسم الأول', 'woocommerce' ),
				'optionalLabel'  => __(
					' الإسم الأول (إختياري)  ',
					'woocommerce'
				),
			...
			'last_name'  => [
				'label'          => __( 'اسم العائلة', 'woocommerce' ),
				'optionalLabel'  => __(
			...
			'address_2'  => [
				'label'          => __( 'شقة، جناح، الخ.', 'woocommerce' ),
				'optionalLabel'  => __(
					'شقة، جناح، الخ (اختياري)',
					'woocommerce'
				),
				'required'       => false,
				'hidden'         => true,
			...
			'state'      => [
				'label'          => __( 'ولاية / مقاطعة', 'woocommerce' ),
				'optionalLabel'  => __(
					'  الولاية/المقاطعة (اختياري) ',
					'woocommerce'
				),
				'required'       => true,
				'hidden'         => false,
			...
			'postcode'   => [
				'label'          => __( 'رمز بريدي', 'woocommerce' ),
				'optionalLabel'  => __(
		...
		if ( ! empty( $field['required'] ) && empty( $value ) ) {
			return new WP_Error(
				...
				// translators: %s is field key.
					__( 'الميدان   %s  مطلوب  .', 'woocommerce' ),
					$field['id']
				)
			);
		}
