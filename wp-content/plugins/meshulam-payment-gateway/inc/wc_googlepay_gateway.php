...
class WC_GooglePay_Meshulam_Gateway extends WC_Payment_Gateway_CC
{
    public $title,
    $description,
    $auth_code,
    ...
    {
        $this->id = 'googlepay-payment';
        $this->icon = MESHULAM_PLUGIN_URL . 'assets/images/gpay.png';
        $this->has_fields = true;
        $this->method_title = __('Google Pay', 'meshulam');
        $this->method_description = __('Pay with Google Pay', 'meshulam');
        $this->supports = [
            'products',
        ];
        ...
        $this->account_details = get_option(
            ...
            array(
                array(
                    'google_greater_then' => $this->get_option('google_greater_then'),
                    'google_less_then' => $this->get_option('google_less_then'),
                    'google_max_payments' => $this->get_option('google_max_payments'),
                ),
            )
        );

        $googlepay_popup = $this->get_option('googlepay_popup_show');
        $this->refund_mode = $this->get_option('refund_mode');
        if ($googlepay_popup == '') {
            $this->update_option('googlepay_popup_show', 'yes');
        }
        ...
        if( get_option('meshulam_googlepay_payment_status') == 1 && $this->get_option('enabled') == ''){
            $this->update_option('enabled', 'yes');
        }

        $this->api_test_mode = $this->get_option('api_test_mode');
        $this->j5_mode = $this->get_option('j5_mode');
        $this->debug = $this->get_option('googlepay_debug');
        $this->title = $this->get_option('title');
        $this->update_url = add_query_arg('wc-api', 'googlepay_server_response', home_url('/'));
        $this->googlepay_pay_title = $this->get_option('googlepay_pay_title');
        ...
        $this->pagecode = $this->api_test_mode == 'yes' ? get_option('meshulam_googlepay_test_pagecode') : get_option('meshulam_googlepay_live_pagecode');
        ...
        $this->response_url = add_query_arg('wc-api', 'googlepay_payment_gateway', home_url('/'));
        ...
        if ($this->j5_mode != 'yes') {
            $this->response_url = add_query_arg('wc-api', 'googlepay_payment_gateway_direct_j4execute', home_url('/'));
            $this->update_url = add_query_arg('wc-api', 'googlepay_server_response_direct_j4execute', home_url('/'));
        }

        add_action('woocommerce_update_options_payment_gateways_' . $this->id, [$this, 'process_admin_options']);
        add_action('woocommerce_update_options_payment_gateways_' . $this->id, array($this, 'save_account_details'));
        add_action('woocommerce_receipt_' . $this->id, [$this, 'receipt_newapi_page']);
        add_action('woocommerce_api_googlepay_payment_gateway', [$this, 'ipn_response']);
        // handle googlepay server response
        add_action('woocommerce_api_googlepay_server_response', [$this, 'googlepay_server_response']);
        // handle ipn response direct j4 execute without j5
        add_action('woocommerce_api_googlepay_payment_gateway_direct_j4execute', [$this, 'ipn_response_direct_j4execute']);
        // handle googlepay server response direct j4 execute without j5
        add_action('woocommerce_api_googlepay_server_response_direct_j4execute', [$this, 'googlepay_server_response_direct_j4execute']);
        add_action('woocommerce_api_googlepay_order_redirect', [$this, 'order_redirect']);
        add_filter('woocommerce_available_payment_gateways', [$this, 'googlepay_available_payment_gateways'], 99, 2);
        add_filter('woocommerce_gateway_title', [$this, 'googlepay_payment_gateway_title'], 103, 2);
    ...
    public function googlepay_payment_gateway_title($title, $payment_id)
    {
        if ($payment_id === 'googlepay-payment' && is_admin()) {
            $title = $this->method_title;
        }
        return $title;
    }
    public function googlepay_available_payment_gateways($available_gateways)
    {
        if (!is_checkout()) {
            return $available_gateways;
        }

        if (array_key_exists('googlepay-payment', $available_gateways)) {
            $available_gateways['googlepay-payment']->order_button_text = $this->googlepay_pay_title;
        }

        $mdetect = new MobileDetect();
        if ($mdetect->isMobile() && $mdetect->isiOS()) {
    ...
    {
        if ($this->googlepay_pay_title != '') {
            return $this->googlepay_pay_title;
        }
        return $button_text;
    }
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = [
            'enabled' => [
                ...
                'type' => 'checkbox',
                'label' => __('Enable Googlepay payment Gateway', 'meshulam-payment-gateway'),
                'default' => 'yes',
            ],
            'title' => [
                        ...
                        <tbody class="accounts">
                            ...
                            if ($this->account_details) {
                                foreach ($this->account_details as $account) {
                                    if (!empty(wp_unslash($account['google_greater_then']))) {
                                        echo '<tr class="account">
                                            <td class="sort"></td>
                                            <td><input type="text" value="' . esc_attr(wp_unslash($account['google_greater_then'])) . '" name="google_greater_then[' . esc_attr($i) . ']" autocomplete="false"/></td>
                                            <td><input type="text" value="' . esc_attr($account['google_less_then']) . '" name="google_less_then[' . esc_attr($i) . ']" autocomplete="false"/></td>
                                            <td><input type="number" class="google_max_payments" value="' . esc_attr(wp_unslash($account['google_max_payments'])) . '" name="google_max_payments[' . esc_attr($i) . ']" autocomplete="false"/></td>
                                            </tr>';
                                        $i++;
                                    }
                                }
                ...
                jQuery(function() {
                    jQuery(document).on('click', 'a.add', function() {
                        ...
                        jQuery('<tr class="account">\
								<td class="sort"></td>\
								<td><input type="text" name="google_greater_then[' + size + ']" autocomplete="false"/></td>\
								<td><input type="text" name="google_less_then[' + size + ']" autocomplete="false"/></td>\
								<td><input type="number" class="google_max_payments" name="google_max_payments[' + size + ']" autocomplete="false"/></td>\
								</tr>').appendTo('#bacs_accounts table tbody');

                        if(max > 0){
                            jQuery(".google_max_payments").attr({"max" : max});
                        }
                        return false;
                    });
                });
    ...
    {
        $accounts = array();
        if (isset($_POST['google_greater_then']) && isset($_POST['google_less_then']) && isset($_POST['google_max_payments'])) {
            $google_greater_then = wc_clean(wp_unslash($_POST['google_greater_then']));
            $google_less_then = wc_clean(wp_unslash($_POST['google_less_then']));
            $google_max_payments = wc_clean(wp_unslash($_POST['google_max_payments']));
            foreach ($google_greater_then as $i => $name) {
                if (empty($google_greater_then[$i])) {
                    continue;
                }
                $accounts[] = array(
                    'google_greater_then' => $google_greater_then[$i],
                    'google_less_then' => $google_less_then[$i],
                    'google_max_payments' => $google_max_payments[$i],
                );
            }
        }
        // phpcs:enable
        update_option('google_account_details', $accounts);
    }
    ...
    {
        $title = __('Googlepay Payment', 'meshulam-payment-gateway');
        $description = __('Googlepay Payment method by Meshulam', 'meshulam-payment-gateway');
    ...
    public function payment_fields()
    {
		...
		<script>
			if (jQuery('label.direct_db').length > 0) {
				jQuery('input#payment_method_googlepay-payment').prop('checked', false);
				jQuery('li.wc_payment_method.payment_method_googlepay-payment').hide();
			}
			if (jQuery('.payment_box.payment_method_googlepay-payment').children('p').length <= 0) {
				jQuery('.payment_box.payment_method_googlepay-payment').addClass('hide_box');
			} else {
				jQuery('.payment_box.payment_method_googlepay-payment').removeClass('hide_box');
			}
		</script>
	<?php	}
    ...
    public function receipt_newapi_page($order_id)
    {
        ...
        $token = unserialize(get_post_meta($order_id, 'googlepay_card_token_detail', true));
        $transaction_id = get_post_meta($order_id, 'googlepay_transaction_id', true);
        ...
        $return_url = add_query_arg('wc-api', 'googlepay_order_redirect', home_url('/'));
        ...
        if (!empty($this->account_details) && count($this->account_details) != 0) {
            foreach ($this->account_details as $account) {
                if (!empty($account['google_less_then']) || !empty($account['google_greater_then'])) {
                    $loop = 'yes';

                    if (!empty($account['google_less_then'])) {
                        $max_val = $account['google_less_then'];
                    } else {
                        $max_val = (float) 1000000;
                    }

                    if (!empty($account['google_greater_then'])) {
                        $min_val = $account['google_greater_then'];
                    } else {
                        $min_val = (int) 0;
                    }
                    ...
                    if (($min_val <= $amount) && ($amount <= $max_val)) {
                        if ($loop_start == true):
                            $i = 1;
                            for ($i = 2; $i <= $account['google_max_payments']; $i++) {
                                $maxPayment = $i;
                            }
                            $loop_start = false;
                        endif;
        ...
        if ($this->debug == 'yes') {
            $logger->add('googlepay_process_payment_newapi', 'Before Sending Data to Meshulam');
            $logger->add('googlepay_process_payment_newapi', 'Request: ' . print_r($args_body, true));
        }
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('googlepay_process_payment_newapi', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('googlepay_process_payment_newapi', 'WP Remote Post Completed');
                $logger->add('googlepay_process_payment_newapi', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
            $args = array($order_id);
    ...
    public function get_payment_page($response, WC_Order $order)
    {
        if (!is_wp_error($response)) {
            ...
            if ($body["status"] == 1) {
                ...
                delete_post_meta($order->get_id(), 'googlepay_card_token_detail');
                delete_post_meta($order->get_id(), 'googlepay_transaction_id');
                update_post_meta($order->get_id(), 'googlepay_card_token_detail', $token_val, true);
                update_post_meta($order->get_id(), 'googlepay_transaction_id', $payment_id, true);
                update_post_meta($order->get_id(), 'meshulam_j5_on', $this->j5_mode, true);
                $order->save();
                ?>
				<div id="meshulam-iframe-container" class="googlepay_payment_iframe">
					<div class="payment_loader"></div><iframe src="<?php echo $body["data"]["url"]; ?>" frameBorder="0"
						id="meshulam-iframe" height="500px" width="100%" allow="payment" scrolling="no"></iframe>
						<script>
...
return;
            ...
            ?>
				<div id="meshulam-iframe-container">
					<h2><?php echo __('Connection error.', 'googlepay-payment-gateway'); ?></h2>
				</div>
				<?php
return;
        }
    ...
    public function order_redirect()
    {
        ...
        $status = get_post_meta($order_id, 'googlepay_transaction_status', true);
        if ($status) {
            //complete order payment
            if (1 == $status) {
                $asmatcha = get_post_meta($order_id, 'googlepay_transaction_asmatcha', true);
                ...
                //mark order as completed after googlepay payment
                delete_post_meta($order_id, 'googlepay_transaction_status');
                $redirect_url = isset($this->thank_you_page) && !empty($this->thank_you_page) ? $this->thank_you_page : $order->get_checkout_order_received_url();
                wp_redirect($redirect_url);
            } else {
                delete_post_meta($order_id, 'googlepay_transaction_status');
    ...
    //googlepay server response direct j4 execute without j5
    public function googlepay_server_response_direct_j4execute()
    {
        if (!empty($_POST)) {
            $logger = wc_get_logger();
            $logger->add('googlepay_notify_response_direct_j4', 'WP Remote Post Response: ' . print_r($_POST, true));
            if ($_POST['status'] == '1' && isset($_POST['data']['customFields']['cField1'])) {
                ...
                if ($status == 'pending' || $status == 'on-hold') {
                    $tr_id = get_post_meta($order_id, 'googlepay_transaction_id', true);
                    $order->add_order_note(sprintf(__('Process created<br/>Process ID: %s', 'meshulam'), $tr_id));
    ...
    public function meshulam_approveTransaction($response)
    {
        if (!empty($response)) {
            ...
            if ($this->debug == 'yes') {
                $logger->add('googlepay_approve_payment_direct_j4', 'Before Sending Data to Meshulam');
                $logger->add('googlepay_approve_payment_direct_j4', 'Request: ' . print_r($args_body, true));
            }
            $args = [
                'method' => 'POST',
                'timeout' => 45,
            ...
            if (is_wp_error($res)) {
                if ($this->debug == 'yes') {
                    $logger->add('googlepay_approve_payment_direct_j4', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
                }
            } else {
                if ($this->debug == 'yes') {
                    ...
                    $data = json_decode($res['body']);
                    $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                    $logger->add('googlepay_approve_payment_direct_j4', 'WP Remote Post Response: ' . print_r($file_data, true));
                }
            }
        }
    }

    //googlepay server response
    public function googlepay_server_response()
    {
        // get googlepay response
        if (!empty($_POST)) {
            $logger = wc_get_logger();
    ...
    public function ipn_response_direct_j4execute()
    {
        if (!empty($_REQUEST)) {
            if ($this->debug == 'yes') {
                $logger = wc_get_logger();
                $logger->add('googlepay_ipn_direct_j4', 'New IPN Fired');
                $logger->add('googlepay_ipn_direct_j4', 'Result: ' . print_r($_REQUEST, true));
            }
            if (isset($_REQUEST['response']) && !empty(isset($_REQUEST['response']))) {
                ...
                if ($_REQUEST['response'] == 'success') {
                    ...
                    if ($error_id == '-81') {
                        $order->update_status('on-hold');
                        $woocommerce->cart->empty_cart();
                        $transaction_id = get_post_meta($_REQUEST['cField1'], 'googlepay_transaction_id', true);
                        ...
                        update_post_meta($order->get_id(), 'googlepay_transaction_id', $new_payment_id, true);
...
exit;
    ...
    public function ipn_response()
    {
        if ($this->debug == 'yes') {
            $logger = wc_get_logger();
            $logger->add('googlepay_ipn', 'New IPN Fired');
            $logger->add('googlepay_ipn', 'Result: ' . print_r($_REQUEST, true));
        }
        $order_id = $_REQUEST['cField1'];
        global $woocommerce;
        $order = wc_get_order($order_id);
        ...
        update_post_meta($order_id, 'googlepay_transaction_status', 1);
        if (isset($_REQUEST['response']) && !empty(isset($_REQUEST['response']))) {
...
exit;
    ...
    public function get_paymentprocessinfo_directj4(WC_Order $order)
    {
        ...
        $order_id = $order->get_id();
        $processToken = unserialize(get_post_meta($order->get_id(), 'googlepay_card_token_detail', true));
        $processId = get_post_meta($order->get_id(), 'googlepay_transaction_id', true);
        ...
        if ($this->debug == 'yes') {
            $logger->add('googlepay_get_payment_process_info_direct_j4', 'Before Sending Data to Meshulam');
            $logger->add('googlepay_get_payment_process_info_direct_j4', 'Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('googlepay_get_payment_process_info_direct_j4', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('googlepay_get_payment_process_info_direct_j4', 'WP Remote Post Completed');
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('googlepay_get_payment_process_info_direct_j4', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
        }
        $body = json_decode($res['body'], true);
        if ($body['status'] == '1') {
    ...
    public function meshulamApprovePayment(WC_Order $order)
    {
        ...
        if ($this->debug == 'yes') {
            $logger->add('googlepay_before_approve_payment', 'WP Remote Post Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('googlepay_before_approve_payment', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('googlepay_before_approve_payment', 'WP Remote Post Completed');
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('googlepay_before_approve_payment', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
        }
        $body = json_decode($res['body'], true);
        if ($body['status'] == '1') {
