...
class WC_Cal_Meshulam_Gateway extends WC_Payment_Gateway_CC
{
    public $title,
    $description,
    $auth_code,
    ...
    public function __construct()
    {
        ...
        $this->method_title = __('CAL', 'meshulam');
        $this->method_description = __('Pay with CAL', 'meshulam');
        $this->supports = [
            'products',
        ];

        $cal_popup = $this->get_option('cal_popup_show');
        $this->refund_mode = $this->get_option('refund_mode');
        if ($cal_popup == '') {
            $this->update_option('cal_popup_show', 'yes');
        }
        ...
        if( get_option('meshulam_cal_payment_status') == 1 && $this->get_option('enabled') == ''){
            $this->update_option('enabled', 'yes');
        }

        $this->api_test_mode = $this->get_option('api_test_mode');
        $this->j5_mode = $this->get_option('j5_mode');
        $this->debug = $this->get_option('cal_debug');
        $this->payment_type = $this->get_option('payment_type');
        $this->maxpayment = $this->get_option('maxpayment');
        $this->title = $this->get_option('title');
        $this->update_url = add_query_arg('wc-api', 'cal_server_response', home_url('/'));
        $this->cal_pay_title = $this->get_option('cal_pay_title');
        ...
        $this->pagecode = $this->api_test_mode == 'yes' ? get_option('meshulam_cal_test_pagecode') : get_option('meshulam_cal_live_pagecode');
        $this->account_details = get_option(
            'cal_account_details',
            array(
                array(
                    'cal_greater_then' => $this->get_option('cal_greater_then'),
                    'cal_less_then' => $this->get_option('cal_less_then'),
                    'cal_max_payments' => $this->get_option('cal_max_payments'),
                ),
            )
        );
        ...
        $this->response_url = add_query_arg('wc-api', 'calpay_payment_gateway', home_url('/'));
        ...
        if ($this->j5_mode != 'yes') {
            $this->response_url = add_query_arg('wc-api', 'cal_payment_gateway_direct_j4execute', home_url('/'));
            $this->update_url = add_query_arg('wc-api', 'cal_server_response_direct_j4execute', home_url('/'));
        }

        add_action('woocommerce_update_options_payment_gateways_' . $this->id, [$this, 'process_admin_options']);
        add_action('woocommerce_update_options_payment_gateways_' . $this->id, array($this, 'save_account_details'));
        add_action('woocommerce_receipt_' . $this->id, [$this, 'receipt_newapi_page']);
        add_action('woocommerce_api_calpay_payment_gateway', [$this, 'ipn_response']);
        // handle cal server response
        add_action('woocommerce_api_cal_server_response', [$this, 'cal_server_response']);
        // handle ipn response direct j4 execute without j5
        add_action('woocommerce_api_cal_payment_gateway_direct_j4execute', [$this, 'ipn_response_direct_j4execute']);
        // handle cal server response direct j4 execute without j5
        add_action('woocommerce_api_cal_server_response_direct_j4execute', [$this, 'cal_server_response_direct_j4execute']);
        add_action('woocommerce_api_calpay_order_redirect', [$this, 'order_redirect']);
        add_filter('woocommerce_available_payment_gateways', [$this, 'cal_available_payment_gateways'], 99, 2);
        add_filter('woocommerce_gateway_title', [$this, 'cal_payment_gateway_title'], 102, 2);
    ...
    public function cal_payment_gateway_title($title, $payment_id)
    {
        if ($payment_id === 'cal-payment' && is_admin()) {
            $title = $this->method_title;
        }
        return $title;
    }
    public function cal_available_payment_gateways($available_gateways)
    {
        if (!is_checkout()) {
            return $available_gateways;
        }

        if (array_key_exists('cal-payment', $available_gateways)) {
            $available_gateways['cal-payment']->order_button_text = $this->cal_pay_title;
        }
    ...
    {
        if ($this->cal_pay_title != '') {
            return $this->cal_pay_title;
        }
        return $button_text;
    }
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = [
            'enabled' => [
                ...
                'type' => 'checkbox',
                'label' => __('Enable CAL payment Gateway', 'meshulam-payment-gateway'),
                'default' => 'yes',
            ],
            'title' => [
                ...
                'default' => __('תשלום באמצעות CAL', 'meshulam-payment-gateway'),
                'desc_tip' => true,
            ],
            'cal_pay_title' => [
            ...
            'cal_popup_show' => array(
                'title' => __('Payment page', 'meshulam-payment-gateway'),
                'type' => 'checkbox',
                'label' => __('Payment via popup', 'meshulam-payment-gateway'),
                'default' => '',
            ),
...
$i = -1;
        if ($this->account_details) {
            foreach ($this->account_details as $account) {
                if (!empty(wp_unslash($account['cal_greater_then']))) {
                    echo '<tr class="account">
										<td class="sort"></td>
										<td><input type="text" value="' . esc_attr(wp_unslash($account['cal_greater_then'])) . '" name="cal_greater_then[' . esc_attr($i) . ']" autocomplete="false"/></td>
										<td><input type="text" value="' . esc_attr($account['cal_less_then']) . '" name="cal_less_then[' . esc_attr($i) . ']" autocomplete="false"/></td>
										<td><input type="number" class="cal_max_payments" value="' . esc_attr(wp_unslash($account['cal_max_payments'])) . '" name="cal_max_payments[' . esc_attr($i) . ']" autocomplete="false"/></td>
										</tr>';
                    $i++;
                }
            }
						...
						jQuery(document).on('click', 'a.add', function() {
							...
							jQuery('#woocommerce_cal-payment_payment_type :selected').each(function(i){
								if(jQuery(this).val() == 'payments_4'){
									max = jQuery('#woocommerce_cal-payment_maxpayment').val();
								}
							});
							...
							jQuery('<tr class="account">\
								<td class="sort"></td>\
								<td><input type="text" name="cal_greater_then[' + size + ']" autocomplete="false"/></td>\
								<td><input type="text" name="cal_less_then[' + size + ']" autocomplete="false"/></td>\
								<td><input type="number" class="cal_max_payments" name="cal_max_payments[' + size + ']" autocomplete="false"/></td>\
								</tr>').appendTo('#bacs_accounts table tbody');

							if(max > 0){
								jQuery(".cal_max_payments").attr({"max" : max});
							}
							return false;
						});
					});
...
return ob_get_clean();
    ...
    {
        $accounts = array();
        if (isset($_POST['cal_greater_then']) && isset($_POST['cal_less_then']) && isset($_POST['cal_max_payments'])) {
            $cal_greater_then = wc_clean(wp_unslash($_POST['cal_greater_then']));
            $cal_less_then = wc_clean(wp_unslash($_POST['cal_less_then']));
            $cal_max_payments = wc_clean(wp_unslash($_POST['cal_max_payments']));
            foreach ($cal_greater_then as $i => $name) {
                if (empty($cal_greater_then[$i])) {
                    continue;
                }
                $accounts[] = array(
                    'cal_greater_then' => $cal_greater_then[$i],
                    'cal_less_then' => $cal_less_then[$i],
                    'cal_max_payments' => $cal_max_payments[$i],
                );
            }
        }
        // phpcs:enable
        update_option('cal_account_details', $accounts);
    }
    ...
    {
        $title = __('CAL Payment', 'meshulam-payment-gateway');
        $description = __('CAL Payment method by Meshulam', 'meshulam-payment-gateway');
    ...
    public function payment_fields()
    {
		...
		<script>
			if (jQuery('label.direct_db').length > 0) {
				jQuery('input#payment_method_cal-payment').prop('checked', false);
				jQuery('li.wc_payment_method.payment_method_cal-payment').hide();
			}
			if (jQuery('.payment_box.payment_method_cal-payment').children('fieldset').length <= 0) {
				jQuery('.payment_box.payment_method_cal-payment').addClass('hide_box');
			} else {
				jQuery('.payment_box.payment_method_cal-payment').removeClass('hide_box');
			}
		</script>

		<?php
if (empty($this->payment_type)) {
            echo '<input type="hidden" name="cal_pay_exp_type" value="2">';
        } else {
            ...
            if (in_array("payments_4", $this->payment_type)) {
                $pay_id = 'payments_4';?>
					...
					<input type="radio" name="cal_pay_exp_type" class="cal_pay_exp_type"
					id="cal_pay_expmonth_<?php echo $pay_id; ?>" value="4" checked="checked">
					<label for="cal_pay_expmonth_<?php echo $pay_id; ?>">
				...
				<div class="form-row form-row-select cal_pay_payments active">
					<label for="cal_pay_exp_numpayment"><span class="required"></span></label>
					<select id="cal_pay_numpayment" name="cal_pay_exp_numpayment" autocomplete="false">
                ...
                if (!empty($this->account_details) && count($this->account_details) != 0) {
                    foreach ($this->account_details as $account) {
                        print_r($account);
                        if (!empty($account['cal_less_then']) || !empty($account['cal_greater_then'])) {
                            $loop = 'yes';
                            if (!empty($account['cal_less_then'])) {$max_val = $account['cal_less_then'];} else { $max_val = (float) 1000000;};
                            if (!empty($account['cal_greater_then'])) {$min_val = $account['cal_greater_then'];} else { $min_val = (int) 0;};
                            if (($min_val <= $amount) && ($amount <= $max_val)) {
                                if ($loop_start == true):
                                    $i = 1;
                                    for ($i = 2; $i <= $account['cal_max_payments']; $i++) {?>
    ...
    public function process_payment($order_id)
    {
        ...
        $exp_type = serialize($_POST['cal_pay_exp_type']);
        if (isset($_POST['cal_pay_exp_numpayment'])) {
            $exp_numpayment = serialize($_POST['cal_pay_exp_numpayment']);
        } else {
            $exp_numpayment = '';
        }
        delete_post_meta($order->get_id(), 'cal_pay_payment-type');
        delete_post_meta($order->get_id(), 'cal_pay_payment-number');
        update_post_meta($order->get_id(), 'cal_pay_payment-type', $exp_type, true);
        update_post_meta($order->get_id(), 'cal_pay_payment-number', $exp_numpayment, true);
        return array(
            'result' => 'success',
    ...
    public function receipt_newapi_page($order_id)
    {
        ...
        $token = unserialize(get_post_meta($order_id, 'calpay_card_token_detail', true));
        $transaction_id = get_post_meta($order_id, 'calpay_transaction_id', true);
        ...
        $return_url = add_query_arg('wc-api', 'calpay_order_redirect', home_url('/'));
        ...
        $shipping_charge = $order->get_total_shipping();
        $tax_total = $order->get_total_tax();
        $pay_num = unserialize($order->get_meta('cal_pay_payment-number'));
        if ($pay_num == '') {$pay_num = 1;}
        $pay_type = unserialize($order->get_meta('cal_pay_payment-type'));
        $this->invoiceNotifyUrl = add_query_arg('order_id', $order_id, $this->invoiceNotifyUrl);
        $args_body = array(
            'pageCode' => $this->pagecode,
        ...
        if ($this->debug == 'yes') {
            $logger->add('cal_process_payment_newapi', 'Before Sending Data to Meshulam');
            $logger->add('cal_process_payment_newapi', 'Request: ' . print_r($args_body, true));
        }
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('cal_process_payment_newapi', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('cal_process_payment_newapi', 'WP Remote Post Completed');
                $logger->add('cal_process_payment_newapi', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
            $args = array($order_id);
        ...
        if (!is_wp_error($response)) {
            ...
            if ($body["status"] == 1) {
				...
				<div class="close_popup_meshulam"><img src="<?php echo MESHULAM_PLUGIN_URL . 'assets/images/close-icon.png'; ?>" height="30" width="30"> </div>
					<div class="payment_loader"><?php echo __('Please do not press back button or refresh', 'meshulam-payment-gateway'); ?></div><iframe src="<?php echo $body["data"]["url"]; ?>" frameBorder="0"
						id="meshulam-iframe" height="467px" width="100%" scrolling="no"></iframe>
						<script>
...
$lang = get_bloginfo('language');
                if ($lang == 'en-US') {
                    $p_title = "Operation failed";
                    $p_short_d = "Please check that the card details are correct";
                    $p_home = "Home";
                    $p_card = "Cart";
                } else {
                ...
                ?>
								const popup = `<div class="meshulam_order_page"><div class="popup_content">
									...
									"><?php echo $p_home; ?></a>
									<a href="<?php echo get_permalink(wc_get_page_id('cart')); ?>"><?php echo $p_card; ?></a>
								</div></div>`;
								jQuery(popup).insertAfter( ".order_details" );
							}
						});
					});
				</script>
...
return;
    ...
    public function order_redirect()
    {
        ...
        $status = get_post_meta($order_id, 'cal_transaction_status', true);
        if ($status) {
            //complete order payment
            if (1 == $status) {
                $asmatcha = get_post_meta($order_id, 'cal_transaction_asmatcha', true);
                ...
                //mark order as completed after cal payment
                delete_post_meta($order_id, 'cal_transaction_status');
                $redirect_url = isset($this->thank_you_page) && !empty($this->thank_you_page) ? $this->thank_you_page : $order->get_checkout_order_received_url();
                wp_redirect($redirect_url);
            } else {
                delete_post_meta($order_id, 'cal_transaction_status');
    ...
    //cal server response direct j4 execute without j5
    public function cal_server_response_direct_j4execute()
    {
        if (!empty($_POST)) {
            $logger = wc_get_logger();
            $logger->add('cal_notify_response_direct_j4', 'WP Remote Post Response: ' . print_r($_POST, true));
            if ($_POST['status'] == '1' && isset($_POST['data']['customFields']['cField1'])) {
                ...
                if ($status == 'pending' || $status == 'on-hold') {
                    $tr_id = get_post_meta($order_id, 'calpay_transaction_id', true);
                    $order->add_order_note(sprintf(__('Process created<br/>Process ID: %s', 'meshulam'), $tr_id));
    ...
    public function meshulam_approveTransaction($response)
    {
        if (!empty($response)) {
            ...
            if ($this->debug == 'yes') {
                $logger->add('cal_approve_payment_direct_j4', 'Before Sending Data to Meshulam');
                $logger->add('cal_approve_payment_direct_j4', 'Request: ' . print_r($args_body, true));
            }
            $args = [
                'method' => 'POST',
                'timeout' => 45,
            ...
            if (is_wp_error($res)) {
                if ($this->debug == 'yes') {
                    $logger->add('cal_approve_payment_direct_j4', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
                }
            } else {
                if ($this->debug == 'yes') {
                    $logger->add('cal_approve_payment_direct_j4', 'WP Remote Post Completed');
                    $data = json_decode($res['body']);
                    $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                    $logger->add('cal_approve_payment_direct_j4', 'WP Remote Post Response: ' . print_r($file_data, true));
                }
            }
        }
    }
    ...
    //cal server response
    public function cal_server_response()
    {
        // get cal response
        if (!empty($_POST)) {
            $logger = wc_get_logger();
    ...
    public function ipn_response_direct_j4execute()
    {
        if (!empty($_REQUEST)) {
            if ($this->debug == 'yes') {
                $logger = wc_get_logger();
                $logger->add('cal_ipn_direct_j4', 'New IPN Fired');
                $logger->add('cal_ipn_direct_j4', 'Result: ' . print_r($_REQUEST, true));
            }
            if (isset($_REQUEST['response']) && !empty(isset($_REQUEST['response']))) {
                ...
                if ($_REQUEST['response'] == 'success') {
                    ...
                    if ($error_id == '-81') {
                        $order->update_status('on-hold');
                        $woocommerce->cart->empty_cart();
                        $transaction_id = get_post_meta($_REQUEST['cField1'], 'calpay_transaction_id', true);
                        ...
                        update_post_meta($order->get_id(), 'calpay_transaction_id', $new_payment_id, true);
...
exit;
    ...
    public function ipn_response()
    {
        if ($this->debug == 'yes') {
            $logger = wc_get_logger();
            $logger->add('cal_ipn', 'New IPN Fired');
            $logger->add('cal_ipn', 'Result: ' . print_r($_REQUEST, true));
        }
        $order_id = $_REQUEST['cField1'];
        global $woocommerce;
        $order = wc_get_order($order_id);
        ...
        update_post_meta($order_id, 'cal_transaction_status', 1);
        if (isset($_REQUEST['response']) && !empty(isset($_REQUEST['response']))) {
...
exit;
    ...
    public function get_paymentprocessinfo_directj4(WC_Order $order)
    {
        ...
        $order_id = $order->get_id();
        $processToken = unserialize(get_post_meta($order->get_id(), 'calpay_card_token_detail', true));
        ...
        if ($this->debug == 'yes') {
            $logger->add('cal_get_payment_process_info_direct_j4', 'Before Sending Data to Meshulam');
            $logger->add('cal_get_payment_process_info_direct_j4', 'Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('cal_get_payment_process_info_direct_j4', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('cal_get_payment_process_info_direct_j4', 'WP Remote Post Completed');
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('cal_get_payment_process_info_direct_j4', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
        }
        $body = json_decode($res['body'], true);
        if ($body['status'] == '1') {
    ...
    public function meshulamApprovePayment(WC_Order $order)
    {
        ...
        $order_id = $order->get_id();
        $processToken = unserialize(get_post_meta($order->get_id(), 'calpay_card_token_detail', true));
        $processId = get_post_meta($order->get_id(), 'calpay_transaction_id', true);
        $pay_sum = $order->get_total();
        ...
        if ($this->debug == 'yes') {
            $logger->add('cal_before_approve_payment', 'WP Remote Post Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('cal_before_approve_payment', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('cal_before_approve_payment', 'WP Remote Post Completed');
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('cal_before_approve_payment', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
        }
        $body = json_decode($res['body'], true);
        if ($body['status'] == '1') {
