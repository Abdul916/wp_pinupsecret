...
class WC_MeshulamPay_Gateway extends WC_Payment_Gateway_CC
{
    ...
    public function __construct()
    {
        ...
        // class setup
        $this->id = 'meshulam-payment';
        $this->icon = MESHULAM_PLUGIN_URL . 'assets/images/company.png';
        $this->has_fields = true;
        $this->method_title = __('grow payment gateway', 'meshulam');
        $this->method_description = __('Pay securely with grow payment gateway', 'meshulam');
        $this->supports = [
            'products',
        ];
        ...
        $this->response_url = add_query_arg('wc-api', 'meshulam_payment_gateway', home_url('/'));
        $this->update_url = add_query_arg('wc-api', 'meshulam_server_response', home_url('/'));
        $this->integromat_send_order_url = "https://hook.enterprise.eu1.integromat.celonis.com/u3ffa4tlwkvfmf36m8brci809jkhzru1";
        if ($this->j5_mode != 'yes') {
            $this->response_url = add_query_arg('wc-api', 'meshulam_payment_gateway_direct_j4execute', home_url('/'));
            $this->update_url = add_query_arg('wc-api', 'meshulam_server_response_direct_j4execute', home_url('/'));
        }
        $this->regular_pagecode = $this->api_test_mode == 'yes' ? get_option('meshulam_regular_test_pagecode') : get_option('meshulam_regular_live_pagecode');
        $this->recurring_pagecode = $this->api_test_mode == 'yes' ? get_option('meshulam_recurring_test_pagecode') : get_option('meshulam_recurring_live_pagecode');

        if( get_option('meshulam_bit_payment_status') == 1 && $this->get_option('enabled') == ''){
        ...
        // handle ipn response
        add_action('woocommerce_api_meshulam_payment_gateway', [$this, 'ipn_response']);
        // handle meshulam server response
        add_action('woocommerce_api_meshulam_server_response', [$this, 'meshulam_server_response']);
        // handle ipn response direct j4 execute without j5
        add_action('woocommerce_api_meshulam_payment_gateway_direct_j4execute', [$this, 'ipn_response_direct_j4execute']);
        // handle meshulam server response direct j4 execute without j5
        add_action('woocommerce_api_meshulam_server_response_direct_j4execute', [$this, 'meshulam_server_response_direct_j4execute']);
        // get invoice url from response meshulam
        add_action('woocommerce_api_meshulam_invoice_response', [$this, 'get_meshulam_invoice_response']);
        //get response of recurring orders list
        add_action('woocommerce_api_latest_meshulam_recurring_response', [$this, 'latest_meshulam_recurring_response']);
        ...
        add_filter('woocommerce_gateway_title', [$this, 'meshulam_payment_gateway_title'], 101, 2);
    ...
    public function meshulam_payment_gateway_title($title, $payment_id)
    {
        if ($payment_id === 'meshulam-payment' && is_admin()) {
            $title = $this->method_title;
        }
        return $title;
    }
    //get response of recurring orders list
    public function latest_meshulam_recurring_response()
    {
        $logger = wc_get_logger();
        $file_data = json_encode($_REQUEST, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        $json = file_get_contents('php://input');
        $json_decode = json_decode($json);
        $json = json_encode($json_decode, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        $logger->add('latest_meshulam_recurring_response', 'Recurring Response: ' . print_r($json, true));
        $logger->add('latest_meshulam_recurring_response', 'Recurring Response(REQUEST): ' . print_r($file_data, true));
        die("exit");
    }
    //get invoice url from response meshulam
    public function get_meshulam_invoice_response()
    {
        if ($this->debug == 'yes') {
            $logger = wc_get_logger();
            $logger->add('invoiceNotifyUrl', 'invoiceNotifyUrl Response: ' . print_r($_REQUEST, true));
        }
        if (isset($_REQUEST['order_id']) && isset($_REQUEST['invoiceUrl'])) {
            $order_id = $_REQUEST['order_id'];
            $invoiceUrl = $_REQUEST['invoiceUrl'];
            update_post_meta($order_id, 'meshulam_invoice_url', $invoiceUrl);
        }
    }
    ...
    public function meshulam_available_payment_gateways($available_gateways)
    {
        ...
        if (array_key_exists('meshulam-payment', $available_gateways)) {
            // Gateway ID for Paypal is 'paypal'. bitpay-payment
            $available_gateways['meshulam-payment']->order_button_text = $this->pay_title;
        }
        return $available_gateways;
    }
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'label' => __('Enable grow payment gateway', 'meshulam-payment-gateway'),
                'type' => 'checkbox',
                'description' => '',
                'default' => 'yes',
            ),
    ...
    {
        $title = __('grow payment gateway', 'meshulam-payment-gateway');
        $description = __('Pay securely with grow payment gateway', 'meshulam-payment-gateway');
    ...
    public function payment_fields()
    {
        ...
        ?>
                    ...
                    ?>
                        ...
                        ?>
                            ...
                            <?php
                                ...
                                <label for="meshulam_pay_expmonth_<?php echo $pay_id; ?>"><span class="required"></span>
                                    <?php _e('Payments', 'meshulam-payment-gateway'); ?>
                                </label>
                    <?php
                            }
                ...
                echo '</fieldset>'; ?>
                <script>
                    function checkTextInPayment() {
                        var _text = jQuery('.payment_method_meshulam-payment .form-row-radio').text()
                        _text = _text.trim();
                        if (_text == '' || !_text || _text == undefined) {
                            jQuery('.payment_box.payment_method_meshulam-payment').remove()
                        }
                    }
                    jQuery(document).ready(function() {
                        ...
                        if (jQuery('.form-row.form-row-select.meshulam_pay_debit.hide_debit_drop.active').length == 0) {
                            jQuery('.payment_method_meshulam-payment #meshulam_pay_expmonth_payments_4').prop("checked", true);
                            jQuery('.payment_method_meshulam-payment .meshulam_pay_payments').addClass('active');
                        }
                        jQuery(document).on('change', '.payment_method_meshulam-payment .meshulam_pay_exp_type', function() {
            ...
            <script>
                jQuery(document).ready(function() {
                    if (jQuery('.payment_box.payment_method_meshulam-payment').children('p').length > 0 || jQuery(
                            '.payment_box.payment_method_meshulam-payment').children('fieldset').length > 0 || jQuery(
                            '#meshulam_pay_expmonth_6').length > 0) {
                        jQuery('.payment_box.payment_method_meshulam-payment').removeClass('hide_box');
                    } else {
                        jQuery('.payment_box.payment_method_meshulam-payment').addClass('hide_box');
                    }
                })
            </script>
            <?php
    ...
    public function receipt_page($order_id)
    {
        ...
        if (get_option('meshulam_payment_version') == 1 && $pay_type == 2 && !empty($card_token_key)) {
            $this->doPaymentWithToken($order, $pay_type, $pay_num);
        } else {
    ...
    public function doPaymentWithNewAPI($order, $pay_type, $pay_num)
    {
        $logger = wc_get_logger();
        $user_id = $order->get_user_id();
        $first_name = $order->get_billing_first_name() . ' ' . $order->get_billing_last_name();
        ...
        if ($this->debug == 'yes') {
            $logger->add('meshulam_process_payment_newapi', 'Before Sending Data to Meshulam');
            $logger->add('meshulam_process_payment_newapi', 'Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_process_payment_newapi', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('meshulam_process_payment_newapi', 'WP Remote Post Completed');
                $logger->add('meshulam_process_payment_newapi', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
    ...
    public function doPaymentWithToken($order, $pay_type, $pay_num)
    {
        if ($this->debug == 'yes') {
            $logger = wc_get_logger();
            $logger->add('meshulam_payment_withtoken', 'New meshulam Payment Process Fired');
        }
        $user_id = $order->get_user_id();
        global $woocommerce;
        //if user logged in and generate orders second time than get cart token
        ...
        if ($this->debug == 'yes') {
            $logger->add('meshulam_payment_withtoken', 'Before Sending Data to Meshulam');
            $logger->add('meshulam_payment_withtoken', 'Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_payment_withtoken', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_payment_withtoken', 'WP Remote Post Completed');
                $logger->add('meshulam_payment_withtoken', 'WP Remote Post Response: ' . print_r($res['body'], true));
            }
            $body = json_decode($res['body'], true);
            if ($body["status"] == 1) {
        ...
        if (!is_wp_error($response)) {
            ...
            ?>
                <div id="meshulam-iframe-container">
                    ...
                    <div class="payment_loader">
                        <?php echo __('Please do not press back button or refresh', 'meshulam-payment-gateway'); ?></div><iframe src="<?php echo $body["data"]["url"]; ?>" frameBorder="0" id="meshulam-iframe" height="467px" width="100%" scrolling="no"></iframe>
                    <script>
                        jQuery(document).ready(function() {
                            window.addEventListener('message', function(e) {
                                if (e.data.action == 'failed_to_load_page') {
                                    <?php
                                    if (isset($this->fail_url) && !empty($this->fail_url)) {
            ...
            ?>
                <div id="meshulam-iframe-container">
                    <div class="close_popup_meshulam"><img src="<?php echo MESHULAM_PLUGIN_URL . 'assets/images/close-icon.png'; ?>" height="30" width="30"> </div>
                    <h2 style="color: #e61717; background: #000; text-align: center; padding: 20px; position: absolute; top: 40%; border-radius: 20px; max-width: 90%; left: 0; right: 0; margin: 0 auto;">
                        <?php echo __('Failed ', 'bit-payment-gateway') . $body["err"]["message"]; ?></h2>
                </div>
            <?php
                return;
            }
    ...
    public function meshulam_approveTransaction($response)
    {
        if (!empty($response)) {
            ...
            if ($this->debug == 'yes') {
                $logger->add('meshulam_approve_payment_direct_j4', 'Before Sending Data to Meshulam');
                $logger->add('meshulam_approve_payment_direct_j4', 'Request: ' . print_r($args_body, true));
            }
            $args = [
                'method' => 'POST',
                'timeout' => 45,
            ...
            if (is_wp_error($res)) {
                if ($this->debug == 'yes') {
                    $logger->add('meshulam_approve_payment_direct_j4', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
                }
            } else {
                if ($this->debug == 'yes') {
                    ...
                    $data = json_decode($res['body']);
                    $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                    $logger->add('meshulam_approve_payment_direct_j4', 'WP Remote Post Response: ' . print_r($file_data, true));
                }
            }
        }
    }
    ...
    public function ipn_response_direct_j4execute()
    {
        if (!empty($_REQUEST)) {
            if ($this->debug == 'yes') {
                $logger = wc_get_logger();
                $logger->add('meshulam_ipn_direct_j4', 'New IPN Fired');
                $logger->add('meshulam_ipn_direct_j4', 'Result: ' . print_r($_REQUEST, true));
            }
    ...
    public function ipn_response()
    {
        ...
        if ($this->debug == 'yes') {
            $logger = wc_get_logger();
            $logger->add('meshulam_ipn', 'New IPN Fired');
            $logger->add('meshulam_ipn', 'Result: ' . print_r($_REQUEST, true));
        }
        global $woocommerce;
        $order = wc_get_order($order_id);
        $status = $order->get_status();
...
<?php
    ...
    public function get_paymentprocessinfo_directj4(WC_Order $order)
    {
        ...
        if ($this->debug == 'yes') {
            $logger->add('meshulam_get_payment_process_info_direct_j4', 'Before Sending Data to Meshulam');
            $logger->add('meshulam_get_payment_process_info_direct_j4', 'Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_get_payment_process_info_direct_j4', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_get_payment_process_info_direct_j4', 'WP Remote Post Completed');
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('meshulam_get_payment_process_info_direct_j4', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
        }
        $body = json_decode($res['body'], true);
        if ($body['status'] == '1') {
    ...
    public function meshulamApprovePayment(WC_Order $order)
    {
        ...
        if ($this->debug == 'yes') {
            $logger->add('meshulam_before_approve_payment', 'Before Sending Data to Meshulam');
            $logger->add('meshulam_before_approve_payment', 'Request: ' . print_r($args_body, true));
        }
        $args = [
            'method' => 'POST',
            'timeout' => 45,
        ...
        if (is_wp_error($res)) {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_before_approve_payment', 'WP Remote Post Error: ' . print_r($res->get_error_message(), true));
            }
        } else {
            if ($this->debug == 'yes') {
                $logger->add('meshulam_before_approve_payment', 'WP Remote Post Completed');
                $data = json_decode($res['body']);
                $file_data = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
                $logger->add('meshulam_before_approve_payment', 'WP Remote Post Response: ' . print_r($file_data, true));
            }
        }
        $body = json_decode($res['body'], true);
        if ($body['status'] == '1') {
